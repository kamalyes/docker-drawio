            ...
            parallel {
                stage ('Draw.io Web & Application server') {
                    ...
                    steps {
                        ...
                        sh "docker run -d --name 'drawio-${BUILD_NUMBER}' -p 18080:8080 -p 18443:8443 ${REPO}:${GIT_COMMIT}"
                        sh "docker ps -a"
                        sleep 10
                        sh "docker logs drawio-${BUILD_NUMBER}"
                        sh 'docker run --rm --link drawio-${BUILD_NUMBER}:drawio blitznote/debootstrap-amd64:17.05 bash -c "curl -i http://drawio:8080/"'
                    }
                    post {
                        always {
